...
const Head = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log('search query', searchCache[searchQuery]);
  ...
  const getSearchSuggestion = async () => {
    ...
    const searchResponse = await fetch(SEARCH_SUGGESTION_API + searchQuery);
  ...
  return (
    <>
      <header className="px-4 flex justify-between items-center py-2 shadow-lg">
        ...
        <div className="w-1/2 flex">
          <div className="relative w-full ">
            <input
              className="border-gray-400 border-1 rounded-l-full w-full p-3"
              type="text"
              value={searchQuery}
            ...
            />
            {showSuggestion && (
              <div className="absolute w-full rounded-lg bg-white shadow-sm">
                <ul>
          ...
          <button className="py-2 px-4 bg-gray-100 border-1 border-gray-400 border-l-0 rounded-r-full">
            🔍
          </button>
        </div>
